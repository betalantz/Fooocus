...
from modules.flags import MetadataScheme, OutputFormat, Performance
...
def get_presets():
    ...
    return presets + [
        f[: f.index(".json")] for f in os.listdir(preset_folder) if f.endswith(".json")
    ]
...
default_loras = get_config_item_or_set_default(
    ...
    and all(
        len(y) == 3
        and isinstance(y[0], bool)
        and isinstance(y[1], str)
        and isinstance(y[2], numbers.Number)
        or len(y) == 2
        and isinstance(y[0], str)
        and isinstance(y[1], numbers.Number)
        for y in x
    ),
)
default_loras = [
...
] + [[True, "None", 1.0] for _ in range(default_max_lora_number - len(default_loras))]
